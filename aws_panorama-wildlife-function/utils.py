...
class Processor():
    def __init__(self, class_names, input_size=640, keep_ratio=False):
        ...
        # outputs per anchor (bbox, confidence and class probabilities)
        self.feature_count = self.class_count + 5
        ...
        self.colors = [[rnd.randint(0, 255) for _ in range(3)]
    ...
    def preprocess(self, image):
        ...
        # Must use a copy here, otherwise it may not work as expected on Panorama
...
def trigger_warning(remote_server_url):
    with urlopen(remote_server_url) as response:
